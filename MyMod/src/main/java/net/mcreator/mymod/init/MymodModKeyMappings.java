...
import net.mcreator.mymod.network.TogglePurifyingFlamesStatMessage;
import net.mcreator.mymod.network.ToggleForesightStatMessage;
import net.mcreator.mymod.network.ToggleEasisesStatMessage;
import net.mcreator.mymod.network.ShowEffectListMenuKeyMessage;
...
public class MymodModKeyMappings {
	public static final KeyMapping TOGGLE_FORESIGHT_STAT = new KeyMapping("key.mymod.toggle_foresight_stat", GLFW.GLFW_KEY_9, "key.categories.misc") {
		...
		public void setDown(boolean isDown) {
			...
			if (isDownOld != isDown && isDown) {
				MymodMod.PACKET_HANDLER.sendToServer(new ToggleForesightStatMessage(0, 0));
	...
	public static final KeyMapping CLEAR_POTION_EFFECTS = new KeyMapping("key.mymod.clear_potion_effects", GLFW.GLFW_KEY_0, "key.categories.misc") {
		...
		public void setDown(boolean isDown) {
			super.setDown(isDown);
			if (isDownOld != isDown && isDown) {
				MymodMod.PACKET_HANDLER.sendToServer(new ClearPotionEffectsMessage(0, 0));
				ClearPotionEffectsMessage.pressAction(Minecraft.getInstance().player, 0, 0);
			}
			isDownOld = isDown;
		}
	};
	public static final KeyMapping TOGGLE_PURIFYING_FLAMES_STAT = new KeyMapping("key.mymod.toggle_purifying_flames_stat", GLFW.GLFW_KEY_8, "key.categories.misc") {
		private boolean isDownOld = false;
		...
		public void setDown(boolean isDown) {
			super.setDown(isDown);
			if (isDownOld != isDown && isDown) {
				MymodMod.PACKET_HANDLER.sendToServer(new TogglePurifyingFlamesStatMessage(0, 0));
				TogglePurifyingFlamesStatMessage.pressAction(Minecraft.getInstance().player, 0, 0);
			}
			isDownOld = isDown;
		}
	};
	public static final KeyMapping TOGGLE_EASISES_STAT = new KeyMapping("key.mymod.toggle_easises_stat", GLFW.GLFW_KEY_7, "key.categories.misc") {
		private boolean isDownOld = false;
		...
		public void setDown(boolean isDown) {
			super.setDown(isDown);
			if (isDownOld != isDown && isDown) {
				MymodMod.PACKET_HANDLER.sendToServer(new ToggleEasisesStatMessage(0, 0));
				ToggleEasisesStatMessage.pressAction(Minecraft.getInstance().player, 0, 0);
			}
			isDownOld = isDown;
		}
	};
	public static final KeyMapping SHOW_EFFECT_LIST_MENU_KEY = new KeyMapping("key.mymod.show_effect_list_menu_key", GLFW.GLFW_KEY_UNKNOWN, "key.categories.misc") {
		private boolean isDownOld = false;
		...
		public void setDown(boolean isDown) {
			super.setDown(isDown);
			if (isDownOld != isDown && isDown) {
				MymodMod.PACKET_HANDLER.sendToServer(new ShowEffectListMenuKeyMessage(0, 0));
